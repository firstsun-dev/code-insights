 ...
 * Discovers and parses sessions from Kilo storage:
 * 1. SQLite database: ~/.local/share/kilo/kilo.db
 * 2. JSON session files: ~/.local/share/kilo/storage/session/<project_id>/*.json
 */
export class KiloProvider implements SessionProvider {
  private debug = process.env.DEBUG?.includes('kilo') || process.env.DEBUG?.includes('*');
  ...
  private parseJsonSafely(content: string, context: string): any {
    try {
      ...
      if (this.debug) {
        console.warn(`[kilo] JSON parse failed for ${context}, attempting repair: ${err}`);
      }
      try {
        const repaired = jsonrepair(content);
        return JSON.parse(repaired);
      } catch (repairErr) {
        console.error(`[kilo] Failed to parse/repair JSON for ${context}: ${repairErr}`);
        throw repairErr;
      }
    }
  }
  ...
  private async discoverDatabaseSessions(options?: { projectFilter?: string }): Promise<string[]> {
    const baseDir = getKiloDir();
    const dbPath = path.join(baseDir, 'kilo.db');
  ...
  private async parseDatabaseSession(virtualPath: string): Promise<ParsedSession | null> {
    ...
    try {
      ...
      // Kilo uses milliseconds timestamps (unlike OpenCode which uses seconds)
      ...
      let toolCallCount = 0;
      let sessionUsageAssigned = false;  // Track if session-level usage has been assigned
      ...
      for (const msgRow of messageRows) {
        ...
        for (const partRow of partRows) {
          ...
          if ((partType === 'text' || partType === 'markdown') && partText) {
            content += (content ? '\n' : '') + partText;
          } else if ((partType === 'reasoning' || partType === 'thinking') && partText) {
            // Kilo uses 'reasoning' instead of 'thinking' - map to thinking
        ...
        // Otherwise, fall back to message-level tokens from step-finish parts or JSON.
        if (!msgUsage) {
          if (sessionRow.tokens_input !== undefined && sessionRow.tokens_input > 0 && type === 'assistant' && !sessionUsageAssigned) {
            // Session-level tokens are totals for the entire session
            // Assign ONLY to the first assistant message so calculateSessionUsage picks them up once
            msgUsage = {
              inputTokens: sessionRow.tokens_input || 0,
              outputTokens: sessionRow.tokens_output || 0,
              cacheCreationTokens: sessionRow.tokens_cache_write || 0,
              cacheReadTokens: sessionRow.tokens_cache_read || 0,
              model,
              estimatedCostUsd: sessionRow.cost || 0,
            };
            sessionUsageAssigned = true;
        ...
        // Kilo timestamps are in milliseconds
        messages.push({
          id: msgRow.id,
          sessionId,
          type,
      ...
      // Kilo session table has: project_id, model (JSON), cost, tokens_input/output/cache_read/cache_write, agent, slug, version, title, directory
      const session: ParsedSession = {
        id: sessionId,
        projectPath: sessionRow.directory || '',
        projectName: sessionRow.title || sessionRow.slug || 'kilo-session',
        summary: null,
  ...
  private async parseJsonSession(filePath: string): Promise<ParsedSession | null> {
    ...
    try {
      ...
      for (const msgFile of messageFiles) {
        ...
        if (fs.existsSync(msgPartsDir)) {
          ...
          for (const partFile of partFiles) {
            ...
            if ((partData.type === 'text' || partData.type === 'markdown') && partData.text) {
              content += (content ? '\n' : '') + partData.text;
            } else if ((partData.type === 'reasoning' || partData.type === 'thinking') && partData.text) {
              // Kilo uses 'reasoning' instead of 'thinking' - map to thinking
              thinking = (thinking ? thinking + '\n' : '') + partData.text;
              ...
              if (!msgData.tokens) {
                msgUsage = {
                  ...
                  cacheCreationTokens: partData.tokens.cache?.write || 0,
                  cacheReadTokens: partData.tokens.cache?.read || 0,
                  model: msgData.model?.modelID || 'unknown',
                  estimatedCostUsd: partData.cost || 0,
                };
              }
            } else {
        ...
        const model = msgData.model?.modelID || 'unknown';
        ...
        // Kilo timestamps are in milliseconds (time.created)
        messages.push({
          id: msgData.id,
      ...
      const session: ParsedSession = {
        ...
        projectName: sessionData.title || sessionData.slug || 'kilo-session',
        summary: null,
